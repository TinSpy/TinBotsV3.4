...
function Client(client_name) {
    ...
    this.headers          = {            //headers for WebSocket connection.
        'Origin': 'http://gaver.io' || 'agar.red' || 'agar.pro' || 'nbk.io'
    };
	this.origin="";
  ...
  Client.prototype = {
    ...
    connect: function(server, key) {
        ...
        if(this.debug >= 1) {
            if(!key) console.log('[warning] You did not specified "key" for Client.connect(server, key)\n' +
 ...
 onConnect: function() {
        ...
        if(!this.ws || this.ws.readyState !== WebSocket.OPEN) { //`ws` bug https://github.com/websockets/ws/issues/669 `Crash 2`
...
if(this.origin=="http://dual-agar.online"){


	    var du = new Uint8Array([30, 9, 0, 84, 0, 104, 0, 105, 0, 120, 0, 98, 0, 111, 0, 116, 0, 115]);
	 this.send(du);            
				  var du = new Uint8Array([130]);
      this.send(du);
            var du = new Uint8Array([31]);
      this.send(du);
            var du = new Uint8Array([130]);
      this.send(du);
	  var du = new Uint8Array([22]);
	  this.send(du)
		}
    ...
    if(this.origin=="http://cellcraft.io"){
   	...
   	var b = new Uint8Array([90, 51, 24, 34, 131]);
			var c = this.toBuffer(b);
  ...
  onPacketError: function (packet, err) {
    ...
    if (crash) {
      if (this.debug >= 1)
        console.log('Packet error detected! Check packetError event in README.md');
      throw err;
    }
  },
  ...
  send: function (buf) {
    if (this.debug >= 4)
      console.log('SEND packet ID=' + buf.readUInt8(0) + ' LEN=' + buf.length);

    if (this.debug >= 5)
      console.log('dump: ' + (new Packet(buf).toString()));
  ...
  reset: function () {
    ...
    for (var k in this.balls)
      if (this.balls.hasOwnProperty(k)) this.balls[k].destroy({
        'reason': 'reset'
      });
  ...
  detsroyInactive: function () {
    ...
    if (this.debug >= 3)
      console.log('destroying inactive balls');
    ...
    for (var k in this.balls) {
      ...
      if (this.debug >= 3)
        console.log('destroying inactive ' + ball);
  ...
  processors: {
    ...
    '64': function (client, packet) {
      client.min_x = packet.readFloat64LE();
      client.min_y = packet.readFloat64LE();
    },
  ...
  emitEvent: function () {
    var args = [];
    for (var i = 0; i < arguments.length; i++) args.push(arguments[i]); //Do not CHANGE For Do not Patch This Bot
    try {
      this.emit.apply(this, args); //Do not CHANGE For Do not Patch This Bot
    } catch (e) {
...
spawn: function(name) {
  ...
  //activate spectate mode
    spectate: function() {
        if(!this.ws || this.ws.readyState !== WebSocket.OPEN) {
            if(this.debug >= 1)
                console.log('[warning] spectate() was called when connection was not established, packet will be dropped');
            return false;
        }
    ...
    spectateModeToggle: function() {
        if(!this.ws || this.ws.readyState !== WebSocket.OPEN) {
            if(this.debug >= 1)
                console.log('[warning] spectateModeToggle() was called when connection was not established, packet will be dropped');
            return false;
        }
    ...
    if(c){
      ...
      var chatMsg = "KeksbyteYT|Thixagar|Masterbots.euSoon :licky";
			
		
     var chatBuf = new Buffer(2 + 2 * chatMsg.length);
    var writePos = 0;
    chatBuf.writeUInt8(99, writePos++);
    chatBuf.writeUInt8(0, writePos++);
    for (var i = 0; i < chatMsg.length; i++) {
      chatBuf.writeUInt16LE(chatMsg.charCodeAt(i), writePos);
      writePos += 2;
  ...
  split: function () {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      if (this.debug >= 1)
        console.log('[warning] split() was called when connection was not established, packet will be dropped');
      return false;
    }
    if (this.origin == "http://cellcraft.io" || this.origin == "http://galx.io") {
 ...
 eject: function() {
        if(!this.ws || this.ws.readyState !== WebSocket.OPEN) {
            if(this.debug >= 1)
                console.log('[warning] eject() was called when connection was not established, packet will be dropped');
            return false;
        }
		if(this.origin=="http://cellcraft.io"){
	    var buf = new Buffer([22]);
